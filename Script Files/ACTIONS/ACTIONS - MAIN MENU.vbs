...
'Logging usage stats
script_end_procedure("If you see this, it's because you clicked a button that, for some reason, does not have an outcome in the script. Contact your alpha user to report this bug. Thank you!")'STATS GATHERING----------------------------------------------------------------------------------------------------
name_of_script = "ACTIONS - MAIN MENU.vbs"
start_time = timer 
...
'END FUNCTIONS LIBRARY BLOCK================================================================================================
'DIALOGS----------------------------------------------------------------------------------------------------
BeginDialog ACTIONS_scripts_main_menu_dialog, 0, 0, 461, 320, "Actions scripts main menu dialog"
  ButtonGroup ButtonPressed
    CancelButton 405, 300, 50, 20
    PushButton 5, 20, 110, 10, "ABAWD FSET Exemption Check", ABAWD_FSET_EXEMPTION_button
    PushButton 5, 35, 90, 10, "ABAWD Screening Tool", ABAWD_screening_button
    PushButton 5, 50, 50, 10, "BILS updater", BILS_UPDATER_button
    PushButton 5, 65, 50, 10, "Check EDRS", CHECK_EDRS_button
    PushButton 5, 80, 110, 10, "Child Support Disregard FIATer", CS_FIAT_button
    ...
    PushButton 5, 210, 70, 10, "Paystubs Received", PAYSTUBS_RECEIVED_button
    ...
    PushButton 5, 295, 90, 10, "Update worker signature", UPDATE_WORKER_SIGNATURE_button
  ...
  Text 125, 20, 320, 10, "--- Double checks a case to see if any possible ABAWD/FSET exemptions exist."
  Text 105, 35, 270, 10, "--- A tool to walk through a screening to determine if client is ABAWD."
  Text 65, 50, 220, 10, "--- Updates a BILS panel with reoccurring or actual BILS received."
  Text 65, 65, 190, 10, "--- sends an EDRS request for a HH member on a case."
  Text 125, 80, 330, 10, "--- FIATS in the CS disregard for MFIP and DWP as described in CM 17.15.03"
  ...
  Text 95, 125, 350, 15, "--- NEW 12/15!!! FIATs out the SHEL Housing Subsidy making the MFIP case eligible for the Housing Grant."
  ...
  Text 85, 210, 370, 20, "--- Enter in paystubs on one dialog, and it puts that information on JOBS (both retrospective and prospective if applicable), as well as the PIC and HC pop-up, and it'll case note the income as well."
  ...
  Text 105, 295, 190, 10, "--- Updates the default worker signature on your scripts."
EndDialog
...
IF buttonpressed = ABAWD_FSET_EXEMPTION_button then call run_from_GitHub(script_repository & "/ACTIONS/ACTIONS - ABAWD FSET EXEMPTION CHECK.vbs")
IF buttonpressed = ABAWD_screening_button then call run_from_GitHub(script_repository & "/ACTIONS/ACTIONS - ABAWD SCREENING TOOL.vbs")
If buttonpressed = BILS_UPDATER_button then call run_from_GitHub(script_repository & "/ACTIONS/ACTIONS - BILS UPDATER.vbs")
If buttonpressed = CHECK_EDRS_button then call run_from_GitHub(script_repository & "/ACTIONS/ACTIONS - CHECK EDRS.vbs")
If buttonpressed = CS_FIAT_button then call run_from_GitHub(script_repository & "/ACTIONS/ACTIONS - CS DISREGARD FIAT.vbs")
If buttonpressed = COPY_PANELS_TO_WORD_button then call run_from_GitHub(script_repository & "/ACTIONS/ACTIONS - COPY PANELS TO WORD.vbs")
